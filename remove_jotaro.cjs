...
content = content.replace(
  /case "jotaro":\s*this\.performJotaroAttack\([^)]+\);\s*return true;/m,
  `case "jotaro": {
...
content = content.replace(
  /case "jotaro":\s*if \(isSuper\) this\.specialOraOraOra\(isPlayer\);\s*else this\.specialStarFinger\(isPlayer\);\s*break;/m,
  `case "jotaro": {
              const fighter = getFighter("jotaro");
...
const perform = extractAndRemoveMethod('performJotaroAttack(\n    isPlayer: boolean,');
const special = extractAndRemoveMethod('private specialStarFinger(isP: boolean) {');
const superAtt = extractAndRemoveMethod('private specialOraOraOra(isP: boolean) {');

fs.writeFileSync('jotaro_methods.txt', [perform, special, superAtt].join('\\n\\n=====\\n\\n'));
