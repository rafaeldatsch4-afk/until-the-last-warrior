...
content = content.replace(
  /case "batman":\s*this\.performBatmanAttack\([^)]+\);\s*return true;/m,
  `case "batman": {
...
content = content.replace(
  /case "batman":\s*if \(isSuper\) this\.specialTheDarkKnight\(isPlayer\);\s*else this\.specialBatarang\(isPlayer\);\s*break;/m,
  `case "batman": {
              const fighter = getFighter("batman");
...
const perform = extractAndRemoveMethod('performBatmanAttack(\n    isPlayer: boolean,');
const special = extractAndRemoveMethod('private specialBatarang(isPlayer: boolean) {');
const superAtt = extractAndRemoveMethod('private specialTheDarkKnight(isPlayer: boolean) {');

fs.writeFileSync('batman_methods.txt', [perform, special, superAtt].join('\\n\\n=====\\n\\n'));
