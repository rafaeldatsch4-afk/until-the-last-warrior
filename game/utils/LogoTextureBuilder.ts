...
export class LogoTextureBuilder {
  ...
  private static drawLogo(
    ...
    // --- 1. Background Crest (Shield / Hexagonal Rounded Plate) ---
    const plateW = w - 24;
    const plateH = h - 20;
    const plateX = 12;
    const plateY = 10;
    ...
    ctx.beginPath();
    ctx.roundRect(plateX + 5, plateY + 5, plateW - 10, plateH - 10, radius - 4);
    ctx.lineWidth = 1.2;
    ...
    // --- 6. Lower Subtitle Ribbon ("★ THE ULTIMATE SHOWDOWN ★") ---
    ctx.save();
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
