...
export class StaticFighter extends Fighter {
  ...
  performSpecial(params: AttackParams): AttackResult {
    ...
    if (bs.cache.audio.exists("sfx_beam")) bs.sound.play("sfx_beam", { volume: 0.8, rate: 0.7 });

    const hand = bs.getHandPosition(isPlayer);
    const disc = bs.add.ellipse(hand.x, hand.y, 40, 10, 0x00ffff).setDepth(5);
    const discGlow = bs.add.ellipse(hand.x, hand.y, 50, 15, 0x00ffff, 0.5).setDepth(4);
  ...
  performSuper(params: AttackParams): AttackResult {
    ...
    bs.log("STATIC BURST!!!");
    if (bs.cache.audio.exists("sfx_beam")) bs.sound.play("sfx_beam", { volume: 1.2, rate: 0.5 });
    
    // Zoom in
    bs.cameras.main.zoomTo(1.2, 500, "Cubic.easeInOut", true);

    // Charge effect
    const charge = bs.add.circle(attacker.x, attacker.y + 100, 10, 0x00ffff, 1).setDepth(5);
    bs.tweens.add({
        targets: charge,
        scale: 6,
        alpha: 0.2,
    ...
    bs.time.delayedCall(1200, () => {
        ...
        for(let i=0; i<5; i++) {
            ...
            const beam = bs.add.rectangle(rx, target.y - 300, 10, 600, 0x00ffff).setOrigin(0.5, 0).setDepth(10).setAlpha(0.8);
            bs.tweens.add({ targets: beam, width: 40, alpha: 0, duration: 300, onComplete: () => beam.destroy() });
        }
