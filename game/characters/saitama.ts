...
export class SaitamaFighter extends Fighter {
  ...
  performAttack(params: AttackParams): AttackResult {
    ...
    if (attackType === "melee") {
      ...
      const dashGlow = bs.add
        .rectangle(
          attacker.x - (attacker.x < target.x ? -20 : 20),
          attacker.y + 100,
          80,
          10,
          0xffffff,
      ...
      bs.tweens.add({
        ...
        onComplete: () => {
          ...
          const wave = bs.add
            .circle(target.x, target.y + 120, 15, 0xffffff, 0.7)
            .setDepth(6);
          bs.tweens.add({
            targets: wave,
          ...
          if (isComboFinisher) {
            const wave2 = bs.add
              .circle(target.x, target.y + 120, 5, 0xffffff, 1)
              .setDepth(6);
            bs.tweens.add({
              targets: wave2,
              scale: 20,
          ...
          bs.takeDamage(
            !isPlayer,
            Math.floor(
              (isComboFinisher ? 35 : 18) * bs.getDamageMultiplier(transLevel),
            ),
          );
      ...
      bs.tweens.add({
        targets: attacker,
        x: attacker.x + (attacker.x < target.x ? 20 : -20),
        duration: 50,
        yoyo: true,
      });
      ...
      bs.time.delayedCall(50, () => {
        ...
        for (let i = 0; i < blastsCount; i++) {
          bs.time.delayedCall(i * 80, () => {
            ...
            const blast = bs.add
              .ellipse(hand.x, waveY, 40, 60, 0xffffff, 0.7)
              .setDepth(5);
            const core = bs.add
              .ellipse(hand.x, waveY, 20, 30, 0xffffff, 1)
              .setDepth(6);
            ...
            for (let j = 0; j < 3; j++) {
              const line = bs.add
                .rectangle(
                  hand.x,
                  waveY + Phaser.Math.Between(-30, 30),
      ...
      onComplete: () => {
        ...
        bs.time.delayedCall(300, () => {
          ...
          bs.time.delayedCall(100, () => {
            ...
            const splitLine = bs.add
              .rectangle(480, target.y + 120, 960, 10, 0xffffff)
              .setDepth(30)
              .setAlpha(1);
            bs.tweens.add({
              targets: splitLine,
            ...
            for (let i = 0; i < 40; i++) {
              const p = bs.add
                .rectangle(
                  ...
                  Phaser.Math.Between(4, 10),
                  Math.random() > 0.5 ? 0xffffff : 0xffaa00,
                )
                .setDepth(20);
              bs.tweens.add({
                targets: p,
  ...
  performSuper(params: AttackParams): AttackResult {
    ...
    for (let i = 0; i < 15; i++) {
      const p = bs.add
        .circle(
          attacker.x + Phaser.Math.Between(-60, 60),
          attacker.y + Phaser.Math.Between(-60, 60),
    ...
    bs.tweens.add({
      ...
      onComplete: () => {
        ...
        // Head glint
        const glow = bs.add
          .circle(attacker.x, attacker.y - 45, 12, 0xffffff, 1)
          .setDepth(20);
