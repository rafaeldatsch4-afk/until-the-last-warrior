...
export function generateSpidermanSprite(scene: Phaser.Scene) {
    ...
    for (let f = 0; f < FRAMES; f++) {
...

      {
        ...
        if (isAttack) {
          ...
          if (isTransformed)
            box(8, 19 + bob, 4, 1, IRON_GOLD); // Gold bracelet
          else box(8, 19 + bob, 4, 1, WEB_COLOR);

          box(21, 15 + bob, 3, 4, BLUE);
          box(20, 19 + bob, 4, 4, RED); // Glove
          if (isTransformed)
            box(20, 19 + bob, 4, 1, IRON_GOLD); // Gold bracelet
          ...
          if (isTransformed)
            box(8, 19 + bob, 3, 1, IRON_GOLD); // Gold bracelet
          else box(8, 20 + bob, 3, 1, WEB_COLOR); // Web line

          box(21, 15 - bob, 3, 4, BLUE);
          box(21, 19 - bob, 3, 5, RED); // Glove
          if (isTransformed)
            box(21, 19 - bob, 3, 1, IRON_GOLD); // Gold bracelet
  ...
  if (!scene.textures.exists("spiderman")) {
    generateForm(0);
  }
  if (!scene.textures.exists("spiderman_ssj")) {
    generateForm(1);
  }
  if (!scene.textures.exists("spiderman_ui")) {
    generateForm(2);
  }
}
