...
export default class MenuScene extends Phaser.Scene {
  ...
  async showChallengesPopup() {
    ...
    const popupBg = this.add
      .rectangle(0, 0, 500, 520, 0x111625)
      .setStrokeStyle(4, 0xf1c40f)
      .setOrigin(0.5);
    const popupTitle = this.add
      .text(0, -225, "DESAFIOS DO DIA", {
        fontSize: "28px",
        fontFamily:
    ...
    const streakBg = this.add
      .rectangle(0, -145, 440, 90, 0x0c0f1d)
    ...
    if (hasClaimedStreakToday) {
      ...
      const claimStreakBox = this.add
        .rectangle(140, -150, 100, 32, 0x2ecc71)
        .setInteractive({ useHandCursor: true });
      const claimStreakTxt = this.add
        .text(140, -150, "COLETAR", {
          fontSize: "13px",
          color: "#ffffff",
          fontStyle: "bold",
        })
    ...
    CHALLENGES.forEach((challenge) => {
      ...
      const chCard = this.add
        .rectangle(0, startY, 440, 64, 0x000000, 0.5)
        .setStrokeStyle(1, 0x555555);
      const chTitle = this.add.text(-200, startY - 14, challenge.title, {
        fontSize: "16px",
        color: "#ffffff",
        fontFamily: "system-ui, -apple-system, 'Roboto', sans-serif",
        resolution: 2,
      ...
      if (p.claimed) {
        ...
        const claimBtnBox = this.add
          .rectangle(150, startY + 10, 100, 28, 0xf1c40f)
          .setInteractive({ useHandCursor: true });
        const claimBtnTxt = this.add
          .text(150, startY + 10, "COLETAR", {
            fontSize: "13px",
            color: "#000000",
            fontStyle: "bold",
          })
    ...
    if (claimableCount > 1) {
      const collectAllBox = this.add
        .rectangle(0, 215, 180, 36, 0x2ecc71)
        .setInteractive({ useHandCursor: true });
      const collectAllTxt = this.add
        .text(0, 215, "COLETAR TUDO", {
          fontSize: "15px",
          color: "#ffffff",
          fontStyle: "bold",
        })
  ...
  createMenuButton(
    ...
    const polyShadow = this.add
      .polygon(4, 4, points, 0x000000, 0.4)
      .setOrigin(0, 0);
