...
export default class TournamentScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Title
    const title = this.add.text(480, 40, 'CHAVES DO TORNEIO', {
      fontSize: '42px',
      color: '#f1c40f',
      fontStyle: 'bold',
      stroke: '#000000',
    ...
    const finalRound = this.gameState.tournamentRounds[this.gameState.tournamentRounds.length - 1];
    if (finalRound.matches[0].winner !== null) {
        ...
        const winBg = this.add.rectangle(480, 470, 600, 100, 0x000000, 0.8).setStrokeStyle(2, 0xf1c40f);
        
        this.add.text(480, 440, `VENCEDOR: ${winnerChar?.name.toUpperCase()}!`, {
            fontSize: '32px', color: '#f1c40f', fontStyle: 'bold', stroke: '#000000', strokeThickness: 4
        }).setOrigin(0.5);
        ...
        if (winnerId === this.gameState.tournamentPlayerCharId) {
            ...
            window.UTLW.save();
            this.add.text(480, 480, '+500 MOEDAS!', { fontSize: '24px', color: '#2ecc71', fontStyle: 'bold' }).setOrigin(0.5);
        }

        this.createBtn(480, 550, 'VOLTAR AO MENU', () => {
            this.scene.start('MenuScene');
        }, 0x3498db);
        
        this.tweens.add({ targets: winBg, y: 470, alpha: {from: 0, to: 1}, duration: 500 });
        return;
    }
  ...
  drawBracket() {
      ...
      rounds.forEach((round, rIndex) => {
          ...
          round.matches.forEach((match, mIndex) => {
              ...
              // Draw match box
              const isPlayerMatch = match.p1 === this.gameState.tournamentPlayerCharId || match.p2 === this.gameState.tournamentPlayerCharId;
              const boxColor = isPlayerMatch ? 0x2c3e50 : 0x111111;
              const strokeColor = isPlayerMatch ? 0xf1c40f : 0x555555;
              
              this.add.rectangle(x, y, 180, 50, boxColor, 0.8).setStrokeStyle(2, strokeColor);
              
              const p1Name = match.p1 !== null ? INITIAL_CHARACTERS.find(c => c.id === match.p1)?.name : '???';
              const p2Name = match.p2 !== null ? INITIAL_CHARACTERS.find(c => c.id === match.p2)?.name : '???';
              
              const p1Color = match.winner === match.p1 && match.p1 !== null ? '#2ecc71' : (match.winner !== null ? '#555555' : '#ffffff');
              const p2Color = match.winner === match.p2 && match.p2 !== null ? '#2ecc71' : (match.winner !== null ? '#555555' : '#ffffff');
              ...
              this.add.text(x, y - 12, p1Name || '???', { fontSize: '14px', color: p1IsPlayer ? '#f1c40f' : p1Color, fontStyle: 'bold' }).setOrigin(0.5);
              this.add.text(x, y + 12, p2Name || '???', { fontSize: '14px', color: p2IsPlayer ? '#f1c40f' : p2Color, fontStyle: 'bold' }).setOrigin(0.5);
              ...
              if (rIndex < rounds.length - 1) {
                  ...
                  const nextY = startY + (60 * Math.pow(2, rIndex + 1) / 2) - 30 + (Math.floor(mIndex / 2) * 60 * Math.pow(2, rIndex + 1) * 2);
                  
                  const lineColor = match.winner !== null ? 0x3498db : 0x333333;
                  
                  this.add.line(0, 0, x + 90, y, x + 90 + (xSpacing - 180)/2, y, lineColor).setOrigin(0);
                  this.add.line(0, 0, x + 90 + (xSpacing - 180)/2, y, x + 90 + (xSpacing - 180)/2, nextY, lineColor).setOrigin(0);
                  this.add.line(0, 0, x + 90 + (xSpacing - 180)/2, nextY, nextX, nextY, lineColor).setOrigin(0);
              }
          });
      });
  }
  ...
  processCurrentRound() {
      ...
      round.matches.forEach((match, index) => {
          if (index !== playerMatchIndex && match.winner === null && match.p1 !== null && match.p2 !== null) {
      ...
      if (playerMatch.winner === null && playerMatch.p1 !== null && playerMatch.p2 !== null) {
          ...
          this.createBtn(480, 480, 'LUTAR!', () => {
              this.scene.start('BattleScene');
          }, 0xe74c3c);
      }
  }

  advanceWinner(winnerId: number, currentRoundIndex: number, matchIndex: number) {
      const rounds = this.gameState.tournamentRounds!;
  ...
  createBtn(x: number, y: number, text: string, onClick: () => void, color: number = 0x3498db) {
    const container = this.add.container(x, y);
    
    const shadow = this.add.rectangle(4, 4, 250, 60, 0x000000, 0.5).setOrigin(0.5);
    const bg = this.add.rectangle(0, 0, 250, 60, color).setStrokeStyle(2, 0xffffff).setOrigin(0.5);
    const innerBg = this.add.rectangle(0, 0, 242, 52, 0x000000, 0.2).setOrigin(0.5);
    
    const txt = this.add.text(0, 0, text, { fontSize: '26px', color: '#ffffff', fontStyle: 'bold', stroke: '#000', strokeThickness: 4 }).setOrigin(0.5);

    container.add([shadow, bg, innerBg, txt]);
    
    // Pulsing effect if it's the fight button
    if (color === 0xe74c3c) {
        this.tweens.add({ targets: container, scale: 1.05, duration: 800, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
    }

    const hitArea = this.add.rectangle(0, 0, 250, 60, 0x000000, 0).setInteractive({ useHandCursor: true });
    ...
    hitArea.on('pointerover', () => {
      ...
      if (color !== 0xe74c3c) this.tweens.add({ targets: container, scale: 1.05, duration: 100 });
    });
    hitArea.on('pointerout', () => {
      bg.setFillStyle(color);
      if (color !== 0xe74c3c) this.tweens.add({ targets: container, scale: 1, duration: 100 });
    });
