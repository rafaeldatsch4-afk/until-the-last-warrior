...
export default class SettingsScene extends Phaser.Scene {
  ...
  downloadSaveData() {
      ...
      // Feedback
      const txt = this.add.text(380, 430, 'DOWNLOADED!', { color: '#00ff00', fontSize: '14px' }).setOrigin(0.5);
          ...
          reader.onload = (event: any) => {
              try {
                  ...
                  if (parsed.coins !== undefined && Array.isArray(parsed.characters)) {
                      ...
                      // Feedback
                      const txt = this.add.text(580, 430, 'SUCCESS! RELOADING...', { color: '#00ff00', fontSize: '14px' }).setOrigin(0.5);
                  ...
                  const txt = this.add.text(580, 430, 'INVALID FILE!', { color: '#ff0000', fontSize: '14px' }).setOrigin(0.5);
                  this.tweens.add({ targets: txt, alpha: 0, duration: 2000, onComplete: () => txt.destroy() });
              }
          };
