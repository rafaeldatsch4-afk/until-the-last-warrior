...
content = content.replace(
  /case "thukuna":\s*this\.performThukunaAttack\([^)]+\);\s*return true;/m,
  `case "thukuna": {
...
content = content.replace(
  /case "thukuna":\s*if \(isSuper\) this\.specialMalevolentShrine\(isPlayer\);\s*else this\.specialCleave\(isPlayer\);\s*break;/m,
  `case "thukuna": {
              const fighter = getFighter("thukuna");
...
const perform = extractAndRemoveMethod('performThukunaAttack(\n    isPlayer: boolean,');
const special = extractAndRemoveMethod('private specialCleave(isPlayer: boolean)');
const superAtt = extractAndRemoveMethod('private specialMalevolentShrine(isPlayer: boolean)');

fs.writeFileSync('thukuna_methods.txt', [perform, special, superAtt].join('\\n\\n=====\\n\\n'));
