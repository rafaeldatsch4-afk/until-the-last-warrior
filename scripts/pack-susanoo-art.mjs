...
const source=process.argv[2] || 'docs/art/itachi-susanoo-source.png';
...
const bodies=components.filter(c=>c.pixels.length>10000).sort((a,b)=>a.y0-b.y0);
for(let row=0;row<3;row++)bodies.splice(row*4,4,...bodies.slice(row*4,row*4+4).sort((a,b)=>a.x0-b.x0));
if(bodies.length!==12)throw new Error(`Expected 12 separate silhouettes, found ${bodies.length}`);
const scale=Math.min(120/Math.max(...bodies.map(b=>b.y1-b.y0+1)),176/Math.max(...bodies.map(b=>b.x1-b.x0+1))),layers=[],preview=[],boxes=[];
for(let f=0;f<12;f++){
 ...
 // Keep the torso centered; the slash frame includes a long forward blade.
 const headX=f===9?530:(b.x0+b.x1)/2;
 ...
 preview.push({input:await sharp(pixels).resize(w*2,h*2,{kernel:'nearest'}).png().toBuffer(),left:f%4*384+192-offset*2,top:Math.floor(f/4)*300+280-h*2});
...
await sharp({create:{width:2304,height:128,channels:4,background:{r:0,g:0,b:0,alpha:0}}}).composite(layers).png().toFile('game/assets/itachi-susanoo-v1.png');
await sharp({create:{width:1536,height:900,channels:4,background:'#1d2635'}}).composite(preview).png().toFile('docs/itachi-susanoo-v1-preview.png');
await writeFile('docs/itachi-susanoo-v1-frames.json',JSON.stringify({frameWidth:192,frameHeight:128,frames:12,order:['idle 0','idle 1','idle 2','idle 3','walk 0','walk 1','walk 2','walk 3','attack 0','attack 1','defend','charge'],sourceBoxes:boxes},null,2)+'\n');
