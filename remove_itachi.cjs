...
content = content.replace(
  /case "itachi":\s*this\.performItachiAttack\([^)]+\);\s*return true;/m,
  `case "itachi": {
...
content = content.replace(
  /case "itachi":\s*if \(isSuper\) this\.specialTsukuyomi\(isPlayer\);\s*else this\.specialAmaterasu\(isPlayer\);\s*break;/m,
  `case "itachi": {
              const fighter = getFighter("itachi");
...
const perform = extractAndRemoveMethod('performItachiAttack(\n    isPlayer: boolean,');
const special = extractAndRemoveMethod('private specialAmaterasu(isP: boolean) {');
const superAtt = extractAndRemoveMethod('private specialTsukuyomi(isP: boolean) {');

fs.writeFileSync('itachi_methods.txt', [perform, special, superAtt].join('\n\n=====\n\n'));
